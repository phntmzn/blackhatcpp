// ============================================================
// tcp_window.cpp — set crafted TCP window to test handling
// ------------------------------------------------------------
// Compile: clang++ -std=c++17 -O2 -o tcp_window tcp_window.cpp
// Usage:   sudo ./tcp_window <src> <dst> <dport> <window>
// ============================================================
...
int main(int argc, char** argv) {
    ...
    tcp->doff = 5;
    tcp->syn = 1;
    tcp->window = htons(std::atoi(argv[4]));

    sendto(s, buf, 40, 0, (sockaddr*)&dst, sizeof(dst));
    std::printf("[+] window=%s sent\n", argv[4]);
    return 0;
}
