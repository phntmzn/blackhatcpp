// ============================================================
// mss_probe.cpp — advertise tiny MSS to test clamping
// ------------------------------------------------------------
// Compile: clang++ -std=c++17 -O2 -o mss_probe mss_probe.cpp
// Usage:   sudo ./mss_probe <src> <dst> <dport>
// ============================================================
...
int main(int argc, char** argv) {
    ...
    tcp->doff = 6; // 24 bytes TCP header
    tcp->syn = 1;
    tcp->window = htons(65535);

    // MSS option: kind=2, len=4, mss=48
    uint8_t* opt = (uint8_t*)(buf + sizeof(iphdr) + sizeof(tcphdr));
    opt[0] = 2; opt[1] = 4; opt[2] = 0; opt[3] = 48;

    sendto(s, buf, 44, 0, (sockaddr*)&dst, sizeof(dst));
    std::printf("[+] MSS=48 sent\n");
    return 0;
}
